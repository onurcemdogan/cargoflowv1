...
function App() {
  ...
  // Organizasyon varsayılanı ayarlardan gelir. Gelişmiş İşlemler'deki geçici
  // seçim YALNIZ o çalışma için geçerlidir ve organization settings değerini
  // DEĞİŞTİRMEZ (aşağıda hiçbir save çağrısı yoktur).
  ...
  async function handleSuratCreateAndPrintForIds(
    ...
    try {
      ...
      // Resmî Sürat istendi ama seçimde Sürat dışı gönderi vardı: güvenli
      // biçimde CargoFlow şablonuna düşüldü. TEK kısa bilgi verilir (aynı
      // mesaj hem banner hem toast olarak TEKRARLANMAZ).
  ...
  async function handlePrintLabelsForIds(
    ...
    const printTemplateDecision = resolveRunLabelTemplate(
      selectedOrders,
      printOptions.templateOverride,
    )
    if (printTemplateDecision.blockedReason) {
      setOrdersState((current) => ({
        ...current,
        ordersMessage: {
          level: 'error',
          message: printTemplateDecision.blockedReason ?? '',
        },
      }))
      return
    }
