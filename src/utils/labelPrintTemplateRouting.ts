...
//
// Girdi:
//   organizationTemplate — organization_settings.shipmentDefaults.labelPrintTemplate
//   templateOverride     — YALNIZ o baskı çalışması için geçici seçim
//                          (Gelişmiş İşlemler menüsü). Ayarı DEĞİŞTİRMEZ.
//
// Çıktı: bu çalışmada kullanılacak şablon + (gerekiyorsa) tek kısa bilgi
// veya güvenli hata. Mesajlar PII/ZPL İÇERMEZ.
...
/**
 * Sipariş Sürat gönderisi mi? Mevcut ön kontrol kuralıyla AYNI:
 * kargo firması belirtilmemişse (tek sağlayıcılı kurulum) Sürat sayılır.
 */
export function isSuratShipmentOrder(order: CargoOrder | undefined): boolean {
  ...
  // Sağlayıcı hiç belirtilmemiş (tek sağlayıcılı kurulum): mevcut create
  // ön kontrolüyle AYNI varsayım.
  return true
}

export interface LabelPrintTemplateDecision {
  /** Bu çalışmada GERÇEKTEN kullanılacak şablon. */
  template: LabelPrintTemplate
 ...
 *  - AÇIK kullanıcı seçimi (menüden) uygulanamıyorsa güvenli HATA döner;
 *    sessizce başka bir şablonla basılmaz.
 ...
 *  - Seçim boşsa karar şablonu değiştirmez (çağıran katman zaten uyarır).
 */
export function resolveLabelPrintTemplateDecision(
  ...
  if (overridden) {
    // Kullanıcı bu şablonu AÇIKÇA seçti: sessiz sağlayıcı değiştirme YOK.
    return {
